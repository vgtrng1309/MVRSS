...
appearance = {"pedestrian": [create_2D_list(GRID_SIZE) for i in range(2)],
              "cyclist":    [create_2D_list(GRID_SIZE) for i in range(2)],
              "car":        [create_2D_list(GRID_SIZE) for i in range(2)]}
...
for seq in seq_list:
    ...
    for i in range(4, 4+n_frame):
        ...
        for k in range(1,4):
            ...
            if (k_labels.shape[0] > 1):
                ...
                for ra_obj_mask, ra_obj_output, ra_box_center in zip(ra_obj_masks, \
                    ...
                    # Appearance
                    if (appearance[class_name[k-1]][0][r][c] is np.nan):
                        appearance[class_name[k-1]][0][r][c] = 1
                    else:
                        appearance[class_name[k-1]][0][r][c] += 1
                ...
                for rd_obj_mask, rd_obj_output, rd_box_center in zip(rd_obj_masks, \
                    ...
                    # Appearance
                    if (appearance[class_name[k-1]][1][r][c] is np.nan):
                        appearance[class_name[k-1]][1][r][c] = 1
                    else:
                        appearance[class_name[k-1]][1][r][c] += 1
                ...
                ra_box_center = np.array([(k_ra_boxes[0][0] + k_ra_boxes[0][2])//2,
                                          (k_ra_boxes[0][1] + k_ra_boxes[0][3])//2])
                rd_box_center = np.array([(k_rd_boxes[0][0] + k_rd_boxes[0][2])//2,
                                          (k_rd_boxes[0][1] + k_rd_boxes[0][3])//2])
        ...
        cv2.imshow("output_ra", np.hstack((ra_viz_frame, np.full((256,30,3), (255,255,255), np.uint8),\
                                                    ra_outputs)))
        cv2.imshow("output_rd", np.hstack((rd_viz_frame, np.full((256,30,3), (255,255,255), np.uint8),\
                                                    rd_outputs)))
...
for cls in class_name:
    ...
    result[cls]["range_angle"]["appearance"] = appearance[cls][0]
    result[cls]["range_doppler"]["appearance"] = appearance[cls][1]
