...
def convert_data_2Dmat_to_histogram(data, box, translation=[]):
    def box_adjustment(box, trans_range=0, trans_angle=0):
        return [box[0]+trans_angle, box[1]+trans_range, box[2]+trans_angle, box[3]+trans_range]
    
    bins=100
    if (len(data.shape) > 2):
        print(data.shape)
        i = 0
        for dat, trans in zip(data, translation):
            print(trans)
            trans_range, trans_angle = trans
            adj_box = box_adjustment(box, trans_range, trans_angle)
            flat_data = dat[adj_box[0]:adj_box[2], adj_box[1]:adj_box[3]].flatten()
            plt.hist(flat_data, bins, alpha=0.5, label=str(i))
            plt.legend(loc="upper right")
            i += 1
        plt.show()
    else:
        trans_range, trans_angle = translation
        adj_box = box_adjustment(box, trans_range, trans_angle)
        flat_data = data[adj_box[0]:adj_box[2], adj_box[1]:adj_box[3]].flatten()
        plt.hist(flat_data, bins)
        plt.show()
