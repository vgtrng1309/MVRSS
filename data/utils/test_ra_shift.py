...
def transition_range(data, data_rv, confmap=None, trans_range=None):
    ...
    range_grid = np.linspace(start=0.0, stop=51.2, num=256)
    ...
    if shift_range != 0:
        ...
        if shift_range < 0:
            ...
            compen_mag = torch.from_numpy(compen_mag[::-1].copy()).view(1, 1, 1, -1, 1)
...
if __name__ == '__main__':
    ...
    for ind in index:
        data = np.load(os.path.join(root_dir, seq, "range_angle_raw/" + str(ind).zfill(6) + ".npy"))
        # print(data.shape)
        data_amp = data.copy()
        boxes = None
        with open(os.path.join(root_dir, seq, box_anno_dir, ra_box_anno_file), "r") as f:
            boxes = json.load(f)
            boxes = boxes[str(ind).zfill(6)]["boxes"]
            boxes = boxes[0]
        bboxes.append(boxes)
        ...
        if (not torch.is_tensor(confmap)):
        ...
        data_amp = data_amp
        data_amp1 = data_shift_range[0, 0, 0, :, :].numpy() #+ data_shift_range[0, 1, 0, :, :] #** 2
        ...
        data_amps.append(data_amp) 
        data_amp1s.append(data_amp1)
    ...
    sz_y = 1
    plt.figure()
    plt.hist(data_amps[0][bboxes[0][0]:bboxes[0][2], bboxes[0][1]:bboxes[0][3]].flatten(), 
             alpha=0.5, bins=100, label="org")
    plt.hist(data_amp1s[0][bboxes[0][0]-trans_range:bboxes[0][2]-trans_range, bboxes[0][1]:bboxes[0][3]].flatten(), 
             alpha=0.5, bins=100, label="pred_range_shift")
    # plt.hist(data_amps[1][bboxes[1][0]:bboxes[1][2], bboxes[1][1]:bboxes[1][3]].flatten(), 
    #          alpha=0.5, bins=100, label="org_range_shift")
    ...
    # plt.imshow(data_amps[1])
    # # plt.imshow(data_amp1[boxes[1]+trans_range-sz_y:boxes[3]+trans_range+sz_y, boxes[0]-sz_x:boxes[2]+sz_x])
    # ax.set_title("Org Range-shift RA image")
